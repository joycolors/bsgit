...
<script type="text/javascript">
	$(document).ready(function(){
		/** 新增   
	    $("#search").fancybox({
	    	'href'  : "${pageContext.request.contextPath}/findUserMsg.action",
	    	'width' : 733,
	        'height' : 530,
	        'type' : 'iframe',
	        ...
	        'onClosed' : function() { 
	        	window.location.href = "${pageContext.request.contextPath}/showUserMsg.action";
	        }
	    }); **/
	...
	/** 模糊查询用户  **/
	function search(){
		$("#submitForm").attr("action", "${pageContext.request.contextPath}/searchUser.action?page=1&userName=" + $("#searchName").val()).submit();
	}
...
//	 			alert(fyXqCode);
	...
	function del(userId){
		...
		if(userId == '') return;
		if(confirm("您确定要删除吗？")){
			$("#submitForm").attr("action", "${pageContext.request.contextPath}/delUser.action?userId=" + userId).submit();			
		}
	}
		...
		if(confirm("您确定要删除吗？")){
			 $.ajax({
				    ...
				    success:function(data){
				        ...
				        window.location.href="${pageContext.request.contextPath}/showUserMsg.action?page=1";
				    }
				})
		}
	}
...
// 				return;
	...
	function batchDelById(){
		...
		       	}
	        ...
	        if (confirm(msg)==true){   
	            var allcheckbox = "";
	            var becheckbox = "";
	            $("input[name=userId]").each(function(){ //遍历table里的全部checkbox
	            ...
	            if(becheckbox.length > 0) //如果获取到
	                ...
	                $.ajax({
					    type:"post",
					    url:"${pageContext.request.contextPath}/batchDelUser.action",
					    data:"userIds=" + becheckbox,
					    success:function(data){
					        alert("删除成功");
					        //window.location.reload();
					        window.location.href="${pageContext.request.contextPath}/showUserMsg.action?page=1";
	...
	function jumpNormalPage(page, flag){
		if(flag == null || flag == ""){
			$("#submitForm").attr("action", "${pageContext.request.contextPath}/showUserMsg.action?page=" + page).submit();
		}else{
			//alert("sasdasd");
			$("#submitForm").attr("action", "${pageContext.request.contextPath}/searchUser.action?page=" + page + "&userName=" + flag).submit();
		}
	...
	function jumpInputPage(totalPage, flag){
		...
		if($("#jumpNumTxt").val() != ''){
			...
			if(flag == null || flag == ""){
				$("#submitForm").attr("action", "${pageContext.request.contextPath}/showUserMsg.action?page=" + pageNum).submit();
			}else{
				//alert("sasdasd");
				$("#submitForm").attr("action", "${pageContext.request.contextPath}/searchUser.action?page=" + pageNum + "&userName=" + flag).submit();
			}
				...
				<div class="ui_tb">
					<table class="table" cellspacing="0" cellpadding="0" width="100%" align="center" border="0">
						...
						<c:forEach items="${pageBean.list}" var="user" varStatus="status">
							<tr>
									<td><input type="checkbox" name="userId" value="${user.userId }" class="acb" /></td>
									<td>${status.count }</td>
									<td>${user.userName }</td>
									<td>${user.userTel }</td>
									<td><fmt:formatDate value="${user.userThisLoginTime }" pattern="yyyy-MM-dd HH:mm:ss"/></td>
									...
									<td>
										...
										<a href="javascript:delById(${user.userId });">删除</a>
									</td>
							</tr>
						</c:forEach>
